...
while serving:
    ...
    if amount <=0:
        print("Invalid amount. Please enter a positive number./n")
        continue
...
notes
    ...
    customers_served += 1
    total_despensed += amount
    print(f"Transaction complete, {name}!/n")
    again = input("Next customer? (Y/N): ")
    if again != "yes":
        serving = False


    print("/n=== DAILY DENOMINATION REPORT ===")
...
print(f"/nCustomers served : {customers_served}")
print(f"Total amount dispensed: ${total_despensed} units")
print("Thank you for using the ATM Cash Dispenser. Goodbye!")